...
from src.rule_based import evaluate_expert_rules_v1, evaluate_expert_rules_v2
from src import config
...
class FalseAlarmRejector:
    ...
    """
    def __init__(self,classifier_version=2):
        # Load or set default rule thresholds
        self.classifier_version = classifier_version
        if classifier_version ==1:
            self.rules = config.REJECTOR_RULES
        else:
            self.rules= config.RULES_v2
        ...
        self.ml_classifier = DecisionTreeClassifier(max_depth=5, random_state=42)
    ...
    def evaluate_expert_rules(self,features):
        if self.classifier_version ==1:
            rule_result = evaluate_expert_rules_v1(features,self.rules)
        else:
            rule_result = evaluate_expert_rules_v2(features,self.rules)
    ...
    def extract_features(self, audio, fs):
        ...
        total_energy = np.sum(np.abs(Pxx)) + 1e-10

        features['mid_band_power'] = np.sum(np.abs(Pxx[mid_mask])) 
        features['mid_band_ratio'] = np.sum(np.abs(Pxx[mid_mask])) /total_energy
        features['low_band_ratio'] = np.sum(np.abs(Pxx[low_mask])) / features['mid_band_power']
        features['high_band_ratio'] = np.sum(np.abs(Pxx[high_mask])) / features['mid_band_power']
        ...
        if len(t_stft) > 0 and len(active_series) > 0:
            ...
            features['harmonic_score'] = min(total_harmonic_frames / len(t_stft)/5,1) 
        else:
