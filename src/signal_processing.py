...
def compute_psd(audio, fs, nperseg=config.N_FFT):
    ...
    # Use overlap of 75% as in legacy code (nperseg * 0.75)
    noverlap = int(nperseg * 0.75)
    f, Pxx = signal.welch(audio, fs, nperseg=nperseg, noverlap=noverlap)
...
def compute_spectrogram_and_peaks(audio, fs, nperseg=config.N_FFT, noverlap=None):
    ...
    if noverlap is None:
        noverlap = int(nperseg * 0.75) # Default to 75% overlap
...
def find_significant_peaks(freqs, psd_db, noise_floor_db,
    ...
    max_h = np.max(psd_db)
    height_threshold = max_h - config.PEAK_HEIGHT_REL

    # Find peaks based on PSD magnitude (Legacy behavior)
    ...
    for p_idx in peaks:
        # Check Frequency Filter (Legacy: 150 - 2000)
        f_val = freqs[p_idx]
        if not (config.MIN_FREQ <= f_val <= config.MAX_FREQ):
            continue
        ...
        # Enforce Minimum SNR (Optional, keep permissive if needed)
        # For legacy compatibility, we might want to skip this or keep it very low.
